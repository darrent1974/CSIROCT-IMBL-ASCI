...
def convert_epics_to_xtract_hdf5( file_flat, file_dark, file_sample, file_output, sample_start=0, sample_stride=1 ):
    logging.debug('convert_epics_to_xtract_hdf5, sample_start = %d, sample_stride = %d', sample_start, sample_stride )
    
    # Open output X-TRACT HDF5 file
    with h5py.File( file_output, 'w') as out_xtract:
        logging.debug('Opened output - %s', file_output )
        group_exchange = out_xtract.create_group( xtract.h5_group_xtract_exchange )
        ...
        with h5py.File( file_sample, 'r') as in_sample:
            ...
            if sample_start == 0 and sample_stride == 1:
                ...
                list_indexes = list( range( sample_start, shape[0], sample_stride ) )
                logging.debug('list_indexes length = %d', len(list_indexes) )

                # Create the output dataset
                dset_out = group_exchange.create_dataset( 'data', (len(list_indexes), shape[1], shape[2]), dtype=dset_in.dtype )
                logging.debug('Sample output dataset shape - %s', str(dset_out.shape) )

                # Create a temporary array to hold a single projection
                projection = numpy.zeros( ( 1, shape[1], shape[2] ), dtype=dset_out.dtype )

                # Copy strided projections to the new dataset
                index_output = 0
                for index_input in list_indexes:
                    # Read
                    dset_in.read_direct( projection, source_sel=numpy.s_[index_input] )

                    # Write
                    dset_out.write_direct( projection, dest_sel=numpy.s_[index_output] )

                    index_output += 1

                logging.debug('copied %d sample projections', len(list_indexes) )
